...
def derivativesHEB(inp0, inp1, t, paramtup, W, K, O, stimnodes):
    ...
    for ii in range(2):
        ...
        for nn in range(Totalcols):
            ...
            if (ii == 0) and (nn in stimnodes): pbar = np.random.normal(800,50)# Stimulated nodes are only those of the first layer
...
def coupval_layer1(inp0samenode, inp1, WL0L1, KL0L1, Krow, Orow, C3, e0, r, v0):
    ...
    for node, (Kval, Oval) in enumerate(zip(Krow, Orow)):
        if Kval != 0 or Oval != 0: 
            ...
            #  MAKE SENSE IN WHAT DO I HAVE TO USE, Y OR Z??? USING Z WOULD MEAN THAT IT IS NOT AFFECTED BY THE SYNAPTIC CONNECTIONS.
    return pa, pb
...
if __name__ == '__main__':
    ...
    gammaW = 0.3; gammaK = 0.3; gammaO =  0.3; vL = 4.9; vU = 0.01; Wmax = 10; Kmax = 20.8; Omax = 6; Tpre = 20 
    ...
    # From now on it will be quite different, we will be observing how does the second layer behave so we will just comment (or write out this part)
