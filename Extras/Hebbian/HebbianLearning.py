...
# Attempt to replicate results in [1] where authors built a network of neural mass models and trained them
# using a Hebbian-like learning.

# [1] CONA, FILIPPO and URSINO, MAURO, A MULTI-LAYER NEURAL-MASS MODEL FOR LEARNING SEQUENCES USING 
# THETA/GAMMA OSCILLATIONS, 2013, International Journal of Neural Systems, vol. 23, n. 03. 
# DOI: 10.1142/S0129065712500360

# Continuation after the BackupLayer0. In this case, a second set of inhibitory weights are added to the second
# layer of the model. These weights only affect the second layer and their objective is to silence all the patterns
# that are not active at a time. The main objective was: exciting element i of pattern j in the first layer
# would have resulted in an excitation of the whole pattern j in the second layer, inhibiting at the same time
# the other patterns, after a certain time duration, pattern j+1 would be excited, in turn inhibiting all the others
# and etc. until all patterns have been explored.

# This was a very interesting model for me and would have loved to replicate results.
# However, multiple limitations, the main one being that the reference paper made use of a different model for the
# neural mass models, more complicated than the JR model used here.
