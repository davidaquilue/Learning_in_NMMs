# HEBBIAN LEARNING V1. In this module, everything related to the first layer works correctly. 
# Backup file in case the main one starts to go wrong. 
# Created on 9/4/2021
...
# Let us begin by developing the first layer and trying to achieve an autoassociative memory. 
# That is weights are trained receiving a series of patterns, these patterns shall be repeated in
# the following layer. If successful training, the patterns will be repeated in the following
# layer with the additional characteristic that exciting an element i of pattern j in layer N will
# result in excitation of the whole pattern j in layer N+1.
...
def S(m,e0,r,v0):
    '''Transformation corresponding to the transformation in the somas of the net average PSP into 
    an average action potential density.'''
...
def derivativesHEB(inp, t, paramtup, W, stimnodes):
    ...
    # Now the input will be a matrix where each row i corresponds to the variables (z0,y0,z1,y1,z2,y2) 
    # of each node i.
    ...
    for nn in range(Totalcols):
        ...
        if nn in stimnodes:  # Stronger drive
            pbar = np.random.normal(400,50)
        else:
            pbar = np.random.normal(50,10)
        # Coupled intensities, we obtain them from a function.
        # Bear in mind that a row i in the W matrix -> weights of the nodes that arrive to node i.
...
def couplingvalHEBV1(inp, Wrow, C3, e0, r, v0):
    '''Obtains the effects of the coupling for each node. 
    First layer of Hebbian learning only coupling is excitatory'''
    pa = 0
...
def HeunHEB(x0, tspan, tstep, fun, funparams, stim_info, W, learnparams, trainingphase):
    ...
    for n in range(nsteps-1):
        ...
        if trainingphase:
            if n > Tpre+5: # Start obtaining running averages after 5 steps have passed.
                # We perform the sweep taking columns as the main dimension and iterating over every element in the column. 
                # This way less running avgs have to be computed.
